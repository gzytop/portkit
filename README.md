...
「处置」列只在这一行**动不了**的时候才写字，颜色只是辅助 —— 色觉障碍用户同样能分辨：

| 处置列 | 行的样式 | 含义 |
|---|---|---|
| 留空 | 常规 / 淡蓝底加粗 | 普通进程，可以放心终止。**淡蓝底加粗的是常用开发端口**，不用筛选就能一眼扫到 |
| 系统保护 | 琥珀色 | 系统关键进程，**默认拒绝终止**。确实要杀得用右键「强制终止」 |
| 内核残留 | 灰色 | `TIME_WAIT` 等待关闭的连接，没有进程可杀，等几十秒自动消失 |

可终止是常态，所以留空——如果每行都写「可终止」，那一整列就成了没有信息量的噪音。
...
├── CHANGELOG.md        # 每版改了什么，Release 说明由它生成
...
**破坏性操作要有视觉分量。** 按钮按危险程度分级：终止（实心红）> 刷新（实心蓝）。早期版本里「刷新」和「终止」都是实心高饱和色，对一个不可撤销的操作来说过于危险。针对单行的操作（详情、复制）不占操作栏——它们本就该从行上发起，所以收在右键菜单里，避免未选中行时界面上摆着两个灰按钮。
...
**先在 [`CHANGELOG.md`](CHANGELOG.md) 里把「未发布」小节改名为本次的版本号并写清改了什么**，然后打一个 `v` 开头的 tag，其余自动完成：
...
流水线会依次：跑测试 → 生成图标 → 打包 exe → 校验产物大小是否合理 → **真实启动 exe 确认能创建窗口** → 计算 SHA256 → 从 CHANGELOG 抽出本版条目生成说明 → 创建 Release 并上传。

最后那一步很关键：PyInstaller 常见的坑是「构建成功但一运行就因缺模块退出」，只检查文件存在是发现不了的。

CHANGELOG 里找不到与 tag 同名的小节时，**流水线会直接失败**。这是刻意的：早期版本的 Release 说明是一份静态模板，v1.0.0 到 v1.2.1 除 SHA256 外逐字节相同，用户根本看不出升级带来了什么。宁可发版中断，也不要再发出一份没有信息的说明。
